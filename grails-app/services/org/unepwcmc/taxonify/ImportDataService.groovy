...
class ImportDataService {
    ...
    def importData() {
		...
		query = """
            CREATE TABLE tmp_brazil_data (kingdom_name varchar,
		...
		//copy Brazil's data
		query = """
            COPY tmp_brazil_data (kingdom_name, phylum_name, class_name,
        ...
        // clear all data
        query = "DELETE FROM meta_data; DELETE FROM taxon;"
        sql.execute(query)
		...
		query = """
            DROP TABLE IF EXISTS tmp_gbif_species_data;
		...
		query = """
            COPY tmp_gbif_species_data (gbif_id, gbif_name, rank, kingdom,
        ...
        query = """
            DROP TABLE IF EXISTS tmp_cites_data;
        ...
        query = """
             COPY tmp_cites_data(species_id, kingdom_name, phylum_name, class_name, order_name, family_name,
	...
	def importDistributions() {
		...
		def query = """
            DROP TABLE IF EXISTS tmp_distributions;
		...
		query = """
             COPY tmp_distributions(brazil_id, regions, country_code, establishment_means, remarks)
			 FROM '${file.absolutePath}'
			 WITH DELIMITER ','
			 ENCODING 'utf-8' CSV HEADER"""
		...
		query = """
            INSERT INTO meta_data(version, taxon_id, value, type, data)
